...
import login  # This must be a separate file named login.py
import os
import shutil
import json
import io
...
if not st.session_state["is_authenticated"]:
    # Call the login page from the imported login module
    login.login_page()
    # Stop the rest of the script until login is successful
    st.stop()
else:
    # --- USER IS LOGGED IN ---
    # This entire block will only run after successful authentication
    ...
    st.markdown("""
        ...
        .user-bubble { background-color: #DCF8C6; padding: 10px; border-radius: 10px; margin: 5px 0; max-width: 80%; align-self: flex-end; text-align: left; }
        .bot-bubble { background-color: #E6E6FA; padding: 10px; border-radius: 10px; margin: 5px 0; max-width: 80%; align-self: flex-start; text-align: left; }
        .thinking-word { color: #FF69B4; font-weight: bold; font-size: 20px; }
        .query-word { color: #1E90FF; font-weight: bold; font-size: 20px; }
            ...
            try:
                ...
                if st.session_state["model_choice"] == "OpenAI better response (paid)":
                    ...
                    if st.session_state.doc_qa_chain:
                        ...
                        bot_reply = "Please upload and process documents to begin chatting in Open Source Mode."
